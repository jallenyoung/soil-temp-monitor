...
DB_PATH = "soil_temps_anycity.duckdb"
DEFAULT_CITY_QUERY = "Erie, PA"
...
st.set_page_config(page_title="Pre-Emergent Tracker", page_icon="🌱", layout="centered")
...
def upsert_reading(conn, location_key, d, t5, t10, source="manual"):
    conn.execute(
        """
        INSERT INTO readings (location_key, reading_date, temp_5cm, temp_10cm, source)
        VALUES (?, ?, ?, ?, ?)
        ON CONFLICT (location_key, reading_date) DO UPDATE SET
            temp_5cm = excluded.temp_5cm,
            temp_10cm = excluded.temp_10cm,
            source = excluded.source
        """,
        [location_key, d, t5, t10, source],
    )
...
def latest_reading_date(conn, location_key):
    row = conn.execute(
        "SELECT MAX(reading_date) FROM readings WHERE location_key = ?", [location_key]
    ).fetchone()
...
def geocode_city(query):
    """Search Open-Meteo's geocoding API for matching cities."""
...
def fetch_openmeteo_range(start_d, end_d, lat, lon):
    """Pull hourly soil temperature (6cm and 18cm point depths) and
    reduce to a daily MINIMUM per depth -- same convention used in the
    OSU/Columbus tracker.
...
def format_result(r):
    ...
    label = ", ".join(parts)
    return f"{label}  ({r['latitude']:.3f}, {r['longitude']:.3f})"


st.title("🌱 Pre-Emergent Tracker")
st.caption(
    "Modeled daily minimum soil temp (Open-Meteo) for any city — tracked against "
    "fall (≤70°F) or spring crabgrass (50–55°F) thresholds"
)
...
chosen_label = st.selectbox("Match", options=list(options.keys()), index=0)
location = options[chosen_label]
location_key = str(location["id"])

set_setting(conn, "last_city_query", city_query)

st.caption(
    f"⚠️ Modeled soil temperature at 6cm (5cm-equivalent) and 18cm (10cm-equivalent) "
    f"point depths for **{location['name']}** — not a physical station sensor."
)
...
last_date = latest_reading_date(conn, location_key)
...
if df.empty:
    ...
    if st.button("Delete selected"):
        delete_reading(conn, location_key, del_date)
        st.rerun()
